...
export function generateReportPDF({ status, issues, recs, devices, zones, prediction, timeline }) {
  ...
  for (let p = 1; p <= pages; p++) {
    ...
    doc.setFontSize(7);
    setText(doc, C.dim);
    doc.text(\"GENERATED BY SIGNALDECK\", M, H - 18);
...
export async function downloadReport(data) {
  ...
  const fname = `signaldeck-report-${new Date().toISOString().slice(0, 10)}.pdf`;
  ...
  if (navigator.share && navigator.canShare) {
    try {
      ...
      if (navigator.canShare({ files: [file] })) {
        await navigator.share({ files: [file], title: \"SignalDeck WiFi Report\" });
        return \"shared\";
      }
    } catch {
